...
def fetch_rescuegroups_pets(
    ...
    params = {
        "limit": str(limit),
        "fields[animals]": ",".join([
            "name", "ageGroup", "breedString", "sex",
            "summary", "pictureThumbnailUrl"
        ]),
        "include": "pictures",
    }
    ...
    picmap = {str(inc.get("id")): (inc.get("attributes") or {})
              for inc in included if inc.get("type") == "pictures"}
    ...
    for item in data:
        ...
        summary = attrs.get("summary") or " • ".join([p for p in [breed, age, sex] if p])
        pets.append({"name": name, "image": img, "summary": summary})
...
def gallery_page():
    ...
    if start_initial == -1 or start_added == -1 or end_added == -1:
        app.logger.warning("Could not find injection points in gallery.html; serving original file.")
        return send_from_directory(d, f, mimetype="text/html")
...
# --- upgrade existing fetch to keep structured fields (age, sex, breed) ---
def fetch_rescuegroups_pets(
    limit: int = DEFAULT_LIMIT,
    species_view: Optional[str] = None,
    need_pics: bool = True,
...
def api_quiz_pets():
    ...
    try:
        pets = fetch_rescuegroups_pets(limit=limit, species_view=species, need_pics=True)
        if age:
